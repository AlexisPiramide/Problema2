...
export default function MonopolyBoard({ positions }) {
    ...
    function getPlayersOnTile(row, col, span = 1) {
        return positions.filter(({ position }) => {
            const cell = getTileForPosition(position);
    ...
    cells.push(
        <div key={`corner-1-1`} className="corner" id="parking" style={{ gridRow: `1 / span 2`, gridColumn: `1 / span 2` }}>
            {getPlayersOnTile(1, 1, 2).map((p, i) => <img key={i} src={`./${p.img}.png`} alt={`player-${p.position}`} className="player-img" />)}
        </div>
    );
    cells.push(
        <div key={`corner-1-12`} className="corner" id="gotojail" style={{ gridRow: `1 / span 2`, gridColumn: `12 / span 2` }}>
            {getPlayersOnTile(1, 12, 2).map((p, i) => <img key={i} src={`./${p.img}.png`} alt={`player-${p.position}`} className="player-img" />)}
        </div>
    );
    cells.push(
        <div key={`corner-12-1`} className="corner" id="jail" style={{ gridRow: `12 / span 2`, gridColumn: `1 / span 2` }}>
            {getPlayersOnTile(12, 1, 2).map((p, i) => <img key={i} src={`./${p.img}.png`} alt={`player-${p.position}`} className="player-img" />)}
        </div>
    );
    cells.push(
        <div key={`corner-12-12`} className="corner" id="go" style={{ gridRow: `12 / span 2`, gridColumn: `12 / span 2` }}>
            {getPlayersOnTile(12, 12, 2).map((p, i) => <img key={i} src={`./${p.img}.png`} alt={`player-${p.position}`} className="player-img" />)}
        </div>
    );

    // Top edge
    for (let col = 3; col <= 11; col++) {
        const playersHere = getPlayersOnTile(1, col);
        cells.push(
    ...
    // Bottom edge
    for (let col = 3; col <= 11; col++) {
        const playersHere = getPlayersOnTile(12, col);
        cells.push(
    ...
    for (let row = 3; row <= 11; row++) {
        const playersHere = getPlayersOnTile(row, 1);
        cells.push(
    ...
    // Right edge
    for (let row = 3; row <= 11; row++) {
        const playersHere = getPlayersOnTile(row, 12);
        cells.push(
...
function getTileForPosition(pos) {
